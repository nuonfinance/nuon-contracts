...
interface ICollateralHub {
    ...
    function _deletePositionInfo(address _user,bytes32 _feed) external; 
    function _reAssignNewOwnerBalances(address _user, address _receiver, bool _hasPosition, uint256 _tokenId,uint256 _feed) external;
    function userposition(address _owner,uint256 _pos) external view returns (uint256,uint256,bytes32,address,uint256,uint256,uint256,uint256);
    ...
    function liquidityBuffer() external view returns(uint256);
...
contract ChubStorageV2 is Ownable {
    ...
    constructor(
        ICollateralHub _chub,
        INUONController _controller
    ) {
        chub = _chub;
        ...
        returns(uint256,uint256,uint256)
    {
        ...
        _amount.mul(controller.getMintingFee(address(chub)))
        .div(100)
        .div(1e18));
        ...
        returns (uint256,uint256,uint256,uint256)
    {
        ...
        if (userAm > 0) {
            ...
            require((1e18 * 1e18 / result * 100) < controller.getGlobalCollateralRatio(address(chub)), "This will liquidate you");
            require(chub.getUserLiquidityCoverage(_user,_amount) > chub.liquidityCheck(), "Increase your liquidity coverage");
    ...
    function mintLiquidityHelper(uint256 _NUONAmountD18) internal view returns(uint256) {
            uint256 nuonValue = _NUONAmountD18.mul(chub.getTargetPeg()).div(1e18);
            uint256 collateralRequired = nuonValue.mul(1e18).div(chub.getCollateralPrice()).div(chub.assetMultiplier());
            uint256 collateralBuffer = collateralRequired.mul(chub.liquidityBuffer()).div(100);
            return(collateralRequired.add(collateralBuffer));
    }
        ...
        returns(uint256,uint256,uint256)
    {
        ...
        uint256 fees = _collateralAmount
            .mul(controller.getRedeemFee(address(chub)))
            .div(100)
            .div(1e18);
    ...
    function burnNUONEstimation(
        uint256 _NUONAmount,
        address _user) public view returns(uint256,uint256,uint256) {
        uint256 ratio = controller.getGlobalCollateralRatio(address(chub));
